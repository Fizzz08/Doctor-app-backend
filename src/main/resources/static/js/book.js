...
function createDoctorCard(doctor) {
    ...
    card.innerHTML = `
        ...
        <div class="grp">
            ...
            <p>${doctor.location}</p>
            <p>${doctor.year_of_exp} years Experience</p>
            <p>₹${doctor.fees} Consultation fee at clinic</p>
        </div>
        <button class="book-btn" onclick="openBookingModal('${doctor.name}', '${doctor.available_days}')">Book Now</button>
    `;
    return card;
}
...
function displayTimeSlots(selectedDate, doctorName) {
    ...
    const availableSlotsFetch = fetch(`/api/v1/doctor/availableSlot?doctorName=${encodeURIComponent(doctorName)}`);
    const bookedSlotsFetch = fetch(`/api/bookAppointment/bookedSlots?doctorName=${encodeURIComponent(doctorName)}&date=${formattedDate}`);
...
async function confirmBooking() {
    try {
        ...
        const doctorResponse = await fetch(`/api/v1/doctor/details?doctorName=${encodeURIComponent(doctorName)}`);
        ...
        // Fetch user details
        const userResponse = await fetch("/userId", { method: "GET", credentials: "include" });
        if (!userResponse.ok) {
        ...
        const bookingResponse = await fetch('/api/bookAppointment/book', {
            method: 'POST',
    ...
    // Check if the response is successful
        if (!bookingResponse.ok) {
            ...
            console.error('Backend Error:', errorDetails);  // Log the error
